        ...
        for(int z = 0; z < 1000; z++){
            ...
            for(int x = 0; x < matriz_ram.length; x++){
                if(matriz_ram[x][1] == pointer){//Modificar R e chances de modificar M
                    ...
                    for(int i = 0; i < matriz_ram.length;i++){
                        if(matriz_ram[clock_pointer][3] == 1){ // R ocupado
                            matriz_ram[clock_pointer][3] = 0;
                            clock_pointer++;
                        }else{
                            matriz_ram[clock_pointer][4] = 0;
                            
                            int atual = matriz_ram[clock_pointer][0];
                            matriz_swap[atual] = matriz_ram[clock_pointer];//Modifica a janela atual na tabela Swap
                            matriz_ram[clock_pointer] = matriz_swap[pointer]; //Adiciona a nova tabela á ram
                        }
